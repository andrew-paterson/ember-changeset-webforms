...
function parseChangesetWebformField(field, customValidators, formSettings) {
  if (!field) {
    return;
  }
  if (!field.fieldId) {
...
function parse(fieldSchema, customValidators, formSettings) {
  ...
  if ((field.cloneFieldSchema || {}).validationRules) {
    ...
    field.clonedFieldBlueprint.validatesOn.forEach(event => {
      ...
      if (!field.validatesOn.find(fieldEvent => fieldEvent === `${event}Clone`)) {
  ...
  field.validatesOn = (field.validatesOn || []).concat(field.alwaysValidateOn || []);
  field.name = field.name || safeName(`${formSettings.formName}-form-${field.fieldId}-field`);
  field.id = field.id || safeName(`${formSettings.formName}-form-${field.fieldId}-field`);
...
export { parseChangesetWebformField as default };
//# sourceMappingURL=parse-changeset-webform-field.js.map
