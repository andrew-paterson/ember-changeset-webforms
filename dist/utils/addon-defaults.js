import Input from '../components/ember-changeset-webforms/fields/input.js';
import Textarea from '../components/ember-changeset-webforms/fields/textarea.js';
import PowerSelect from '../components/ember-changeset-webforms/fields/power-select.js';
import PowerDatepicker from '../components/ember-changeset-webforms/fields/power-datepicker.js';
import Checkbox from '../components/ember-changeset-webforms/fields/checkbox.js';
import RadioButtonGroup from '../components/ember-changeset-webforms/fields/radio-button-group.js';
import CheckboxGroup from '../components/ember-changeset-webforms/fields/checkbox-group.js';
import Clicker from '../components/ember-changeset-webforms/fields/clicker.js';
import StaticContent from '../components/ember-changeset-webforms/fields/static-content.js';
...
var addonDefaults = {
  ...
  formSettings: {
    ...
    formName: null,
    // String. Must be unique. Used as a namespace for things like input ID and 'for' attributes..
    novalidate: true,
    // Disable the browser's native validation feedback
    hideSubmitButton: false,
    // Boolean - hides the submit button if true
    submitButtonText: 'Submit',
    // String - text to show on the submit form button
    ...
    // Boolean - whether or not to show the button that will empty all fields.
    clearFormButtonText: 'Clear form',
    // String - text to show on the clear form button TODO implement
    resetFormButton: false,
    // Boolean - if true, a button is shown which call the changeset.rollback() method. See https://github.com/poteto/ember-changeset#rollback
    resetFormButtonText: 'Discard changes',
    submitAfterClear: false,
    // Boolean. If true submits, the form after the clear form button is clicked. An example use case is a filters form with a clear filters button, where the desired behaviour is to clear the form fields, and then submit the empty form to reset the filters.
    clearFormAfterSubmit: false,
    // Boolean or string - if true, all fields are reset to their defaults after a the form submitData returns successfully.
    submitButtonType: 'button',
    // String - the type of the submit button. Can be 'button' or 'submit'.
    attrsFromConfig: {
  ...
  fieldSettings: {
    ...
    fieldId: null,
    // String (required) - the id of the field. If not set, an error will be thrown.
    fieldLabel: null,
    // String (required) - the label of the field as a whole. If not set, an error will be thrown. Use hidelabel in order to hide the label. Even if the label is hidden, this value is used to populate accesibility attributes, such as aria-label.
    propertyName: null,
    // Optional, defaults to the value oif fieldId if not set.
    name: null,
    // String - defaults to the fieldId
    // BEGIN-SNIPPET generic-field-settings.js
    validationRules: [],
    // Array of objects defining validation rules. See "Validation".
    validatesOn: ['submit', 'pushErrors'],
    // Array of strings, specifing the names of events which should triggger validation.
    showValidationWhenFocussed: null,
    // Boolean - unless this is true, validation colours, icons and messages will be omitted for as long as the "focussed" prop of a field is true. The build in input and textarea fields set focussed to true when the user focuesses the element.
    omitted: false,
    // Boolean - or object - if true, the field is omitted and also ignored when validating or submitting the form. Can also be an object which defines rules for dynamic omission/inclusion. See /docs/hiding-and-showing-fields.
    defaultValue: null,
    // Any - auto set the changeset property for the field to this value when the ChangesetWebform component is rendered and the changeset is created. This value will be overridden by a corresponding property in the data object that is passed to the ChangesetWebform component.
    labelComponent: null,
    // Object with { componentClass, props }.
    ...
    // `@changesetWebform, and @formField are passed to the component.
    labelMarkdown: null,
    // String - a markdown string to render as HTML within the label element.
    hideLabel: null,
    // Boolean - Hide the label from the user
    disabled: false,
    // Boolean - disable the field, but do not hide it. It will still be validated [TODO check] and included when the form is submitted
    resetWhenOmitted: true,
    // Boolean - reset the field value and validation when the field is omitted from the form.
    attrsFromConfig: {
      classNames: {} // Object - keys can correspond to those in the classNames settings. See /docs/configure-classnames
    },
    cloneActionsPosition: 'fieldActions',
    // String - where to place the remove clone button in relation to the cloned field. Can be [TODO]
    includeLabelForAttr: false,
    // Boolean - if true, the label element will have a 'for' attribute that matches the input element's 'id' attribute.
    isFieldset: false,
    // Boolean - if true, the field options are wrapped in a fieldset element, and the field label is wrapped in a legend element.
  ...
  fieldTypes: [{
    ...
    fieldType: 'input',
    inputType: 'text',
    // String - the html input type
    autofocus: false,
    // Boolean - whether to autofocus the input on insert
    placeholder: null,
    // String - placeholder text of the input
    trim: true,
    // Trim spaces from the beginning and end of the input after focus out. This is never applied to inputs with type password, even if true.
    includeLabelForAttr: true,
    // Boolean - if true, the label element will have a 'for' attribute that matches the input element's 'id' attribute.
    validatesOn: ['$inherited', 'focusOut'],
    // Array of strings
    // END-SNIPPET
    ...
    // `@changesetWebform and @formField are passed to the component.
    minClones: 1,
    // Number - minimum number of clones allowed.
    maxClones: null,
    // Number - maximum number of clones allowed.
    cloneButtonText: null,
    // String - text to show in the add clone button. Defaults to `Add ${clonedField.fieldLabel} field`
    cloneFieldSchema: {},
    // Object - the field definition of the clones, defined in the same way that you would define the field as a one off field.
    validatesOn: ['$inherited', 'removeClone'],
    // Array of strings
    cloneGroupActionsPosition: 'cloneGroupWrapper',
    // String. Can also be labelWrapper, If cloneGroupWrapper, the clone group action buttons and content will appear below the cloned fields. If `labelWrapper` the the field bale will be wrapper in a div, and the clone group action buttons will be rendered in the label wrapper, after the label element.
    ...
    fieldType: 'powerSelect',
    allowClear: false,
    // Boolean. If true, the select box shows a clear icon which clears the value oif the field. See https://ember-power-select.com/docs/the-trigger for more.
    searchEnabled: false,
    // Boolean. If true, a search box will display at the top of the select options, and will filter the options list then the user types. See https://ember-power-select.com/docs/the-search for more.
    searchPlaceholder: 'Search',
    // String. If passed it will replace the default placeholder in the search box for the power select list.
    options: [],
    // Array of items to show in the dropdown. Items can either all be objects, or they can all be primitives, such as strings or numbers. If an array of objects is passed, then optionDisplayProp should be passed to determine which property in the object should be shown as the label of the option in the list.
    optionDisplayProp: null,
    // String - which property of the object to show in the list if options is an array of objects.
    ...
    // The imported class of the component to pass to the Power Select component. See https://ember-power-select.com/docs/api-reference
    validatesOn: ['$inherited', 'valueUpdated'],
    // Array of strings
    requiresAriaLabelledBy: true,
    // END-SNIPPET
    componentClass: PowerSelect
  }, {
    // BEGIN-SNIPPET powerDatePicker-field-options.js
    fieldType: 'powerDatePicker',
    dateTimeFormat: 'YYYY-MM-DD HH:mm:ss',
    // String - time format to use
    dateTimeDisplayFormat: null,
    // String - the format of the datetime to show in the trigger input. Defaults to dateTimeFormat if null.
    defaultTime: '00:00:00.000',
    // String - default time. Must be in the format HH:mm:ss.SSS.
    fixedTime: null,
    // String - force the time to a value, whatever tha date is. Must be in the format HH:mm:ss.SSS
    showTimeSelector: false,
    // Boolean - show the UI for the user to change the time.
    timeSelectorFields: 'HH,mm,ss,SSS',
    // String - comma separated list of the fields to show in the time selector component. combination of valid momentjs time string parts can be given.
    ...
    minDate: null,
    // String - the earliest day that the calendar will allow the user to select. Must be in the format YYYY-MM-DD.
    maxDate: null,
    // String - the latest day that the calendar will allow the user to select. Must be in the format YYYY-MM-DD.
    validatesOn: ['$inherited', 'valueUpdated', 'blurDateTimeInput'],
    // Array of strings
    requiresAriaLabelledBy: true,
    // END-SNIPPET
    componentClass: PowerDatepicker,
    customParser(field) {
    ...
    checkboxLabelMarkdown: null,
    // Markdown string - a markdown string to render as HTML TODO doc what addon is needed to use this and add to all the other labels.
    validatesOn: ['$inherited', 'checkboxToggled'],
    // Array of strings
    // END-SNIPPET
    ...
    fieldType: 'radioButtonGroup',
    options: [],
    // Array of objects, see /docs/radio-button-group#radio-button-group-options-prop
    optionLabelComponent: null,
    // Object with { componentClass, props }.
    // `componentClass` is the imported class of the component to replace the label element for each option.
    // `props` can be included to pass state or data to the component, accessible as {{@props}}.
    // `@option`, `@for`, `@labelId`, `@checked`, `@changesetWebform`, and `@formField` are also passed to the component.
    // Set `for={{@for}}` and `id={{@labelId}} on the label element in the component to ensure accessibility.
    validatesOn: ['$inherited', 'radioOptionChanged'],
    // Array of strings
    isFieldset: true,
    // Wrap field options in a fieldset element and field label in a legend element.
    // END-SNIPPET
    componentClass: RadioButtonGroup
  }, {
    // BEGIN-SNIPPET checkboxGroup-field-options.js
    fieldType: 'checkboxGroup',
    options: [],
    // Array of objects, docs/checkbox-group#checkbox-group-options.
    optionLabelComponent: null,
    // Object with { componentClass, props }.
    // `componentClass` is the imported class of the component to replace the label element for each option.
    // `props` can be included to pass state or data to the component, accessible as {{@props}}.
    ...
    // Wrap field options in a fieldset element and field label in a legend element.
    // END-SNIPPET
    componentClass: CheckboxGroup
  }, {
    // BEGIN-SNIPPET clicker-field-options.js
    fieldType: 'clicker',
    clickerText: null,
    // String - text to display in the clicker element.
    ...
    validatesOn: ['$inherited', 'valueUpdated'],
    // Array of strings
    triggerComponent: PowerSelectCheckboxesTriggerComponent,
...
export { addonDefaults as default };
//# sourceMappingURL=addon-defaults.js.map
