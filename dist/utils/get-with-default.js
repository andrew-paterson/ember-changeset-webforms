...
const addonDefaults = {
  ...
  formSettings: {
    ...
    submitButtonIconComponent: null,
    ...
    // Boolean or string - if true, all fields are reset to their defaults after a the form submitAction returns successfully. If set to `suppressDefaultValues` all fields will br cleared.
    showClearFormButton: false,
    // Boolean - whether or not to show the button that will empty all fields TODO check if this works
    ...
    showRollbackChangesetButton: false,
  ...
  fieldSettings: {
    ...
    validatesOn: [],
    // Array of strings, possible values include focusOut, keyUp, onChange // TODO check onChanger as validation event
    alwaysValidateOn: ['submit'],
    // Array of strings
    showValidationWhenFocussed: false,
    // Boolean - unless this is tru, validation colours, icons and messages will be hidden for as long as the "focussed" prop of a field is true. The build in input and textarea fields set focussed to true when the user focuesses the element.
    hideSuccessValidation: false,
    // Boolean - only show validation colours when field validation fails
    hidden: false,
    // Boolean - if true, the field is hidden and also ignored when validating or submitting the form
    castOut: false,
    // Boolean - exclude the field from validation and submission
    defaultValue: null,
    ...
    fieldLabel: null,
    // String - the label to show on the field
    labelComponent: null,
    // Object with { componentClass, props }.
    ...
    classNames: {},
    // Object - keys can correspond to those in the classNames settings. See /docs/configure-classnames
  ...
  fieldTypes: [{
    ...
    // Boolean - if true, the label element will have a 'for' attribute that matches the input element's 'id' attribute.
    alwaysValidateOn: ['focusOut', 'valueUpdated'],
    // Array of strings
    // END-SNIPPET
    componentClass: ensureSafeComponent(Input)
  }, {
    ...
    removeCloneComponent: {
      componentPath: ensureSafeComponent(IconTrashComponent)
    },
    // Object with { componentClass, props }.
    ...
    addCloneButtonComponent: {
      componentPath: ensureSafeComponent(AddCloneButtonComponent)
    },
    // Object with { componentClass, props }.
    ...
    // Boolean - if true, the label element will have a 'for' attribute that matches the input element's 'id' attribute.
    // END-SNIPPET
    componentClass: ensureSafeComponent(Textarea)
  }, {
    ...
    options: [],
    // Array of items. Items ban be of any type, but they must all be the same type. If an array of objects ios passed, then optionDisplayProp can be passed to determine which property in the object should be shown as the label of the option in the list.
    optionDisplayProp: null,
    // String - if options is an array of objects, provide the key to show in the list
    ...
    alwaysValidateOn: ['valueUpdated'],
    // Array of strings
    // END-SNIPPET
    componentClass: ensureSafeComponent(PowerSelect)
  }, {
    ...
    alwaysValidateOn: ['valueUpdated', 'blurDateTimeInput'],
    // Array of strings
    // END-SNIPPET
    componentClass: ensureSafeComponent(PowerDatepicker),
    customParser(field) {
      // TODO document this
    ...
    // Array of strings
    // END-SNIPPET
    componentClass: ensureSafeComponent(Checkbox)
  }, {
    ...
    // END-SNIPPET
    componentClass: ensureSafeComponent(RadioButtonGroup)
  }, {
    ...
    // Wrap field options in a fieldset element and field label in a legend element.
    // END-SNIPPET
    componentClass: ensureSafeComponent(CheckboxGroup)
  }, {
    ...
    // END-SNIPPET
    componentClass: ensureSafeComponent(Clicker)
  }, {
    ...
    // `@changesetWebform and @formField are passed to the component.
    // END-SNIPPET
    componentClass: ensureSafeComponent(StaticContent)
  }, {
    ...
    alwaysValidateOn: ['valueUpdated'],
    // Array of strings
    triggerComponent: ensureSafeComponent(PowerSelectCheckboxesTriggerComponent),
...
function getWithDefault(appDefaults = {}, formSchema = {}) {
  ...
  const mergedFields = (formSchema.fields || []).map(field => {
    ...
    const mergedAddonDefaults = _mergeWith({}, addonFieldDefaults, addonFieldTypeDefaults, concatArrayCustomizer);
    const mergedAppDefaults = _mergeWith({}, appConfigFieldDefaults, appConfigFieldTypeDefaults, concatArrayCustomizer);
    const mergedInstanceDefaults = _mergeWith({}, formSchema.fieldSettings, field, concatArrayCustomizer);
    const mergedField = _mergeWith({}, mergedAddonDefaults, mergedAppDefaults, mergedInstanceDefaults, replaceArrayCustomizer);
...
export { addonDefaults, getWithDefault as default };
