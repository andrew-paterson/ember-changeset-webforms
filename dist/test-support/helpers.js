import { find, findAll, waitUntil, click } from '@ember/test-helpers';
...
var helpers = {
  ...
  async passedValidation(arg, assert, opts = {}) {
    ...
    const validatingElement = element.querySelector('.form-control') || element.querySelector('.form-check-input');
    const validatingElementType = element.querySelector('.form-control') ? 'control' : 'check';
    const validatingElementCSS = window.getComputedStyle(validatingElement, null);
    if (assert) {
      assert.dom(validatingElement).hasClass(validClass, `Form ${validatingElementType} element has class '${validClass}' => ${opts.assertionSuffix}`);
      assert.dom(validatingElement).doesNotHaveClass(invalidClass, `[Field passed validation] Form ${validatingElementType} element does not have class '${invalidClass}' => ${opts.assertionSuffix}`);
      assert.strictEqual(validBorderColour, validatingElementCSS.getPropertyValue('border-color'), `[Field passed validation] Form ${validatingElementType} element has valid border colour => ${opts.assertionSuffix}`);
      if (validatingElementType === 'control') {
        assert.strictEqual(validBackgroundImage, validatingElementCSS.getPropertyValue('background-image'), `[Field passed validation] Form ${validatingElementType} element has valid background image => ${opts.assertionSuffix}`);
      }
      if (validatingElementType === 'check') {
        assert.strictEqual(validBorderColour, validatingElementCSS.getPropertyValue('background-color'), `[Field passed validation] Form ${validatingElementType} element has valid background colour => ${opts.assertionSuffix}`);
      }
    }
    if (validatingElement.classList.contains('is-valid') && validatingElementCSS.getPropertyValue('border-color') === validBorderColour && validatingElementCSS.getPropertyValue('background-image') === validBackgroundImage) {
      return true;
    }
    return false;
  },
  async failedValidation(arg, assert, opts = {}) {
    ...
    const validatingElementCSS = window.getComputedStyle(validatingElement, null);
    if (assert) {
      assert.dom(validatingElement).hasClass(invalidClass, `[Field failed validation] Form ${validatingElementType} element has class '${invalidClass}' => ${opts.assertionSuffix}`);
      assert.dom(validatingElement).doesNotHaveClass(validClass, `[Field failed validation] Form ${validatingElementType} element does not have class '${validClass}' => ${opts.assertionSuffix}`);
      assert.strictEqual(invalidBorderColour, validatingElementCSS.getPropertyValue('border-color'), `[Field failed validation] Form ${validatingElementType} element has invalid border colour => ${opts.assertionSuffix}`);
      if (validatingElementType === 'control') {
        assert.strictEqual(invalidBackgroundImage, validatingElementCSS.getPropertyValue('background-image'), `[Field failed validation] Form ${validatingElementType} element has invalid background image => ${opts.assertionSuffix}`);
      }
      if (validatingElementType === 'check') {
        assert.strictEqual(invalidBorderColour, validatingElementCSS.getPropertyValue('background-color'), `[Field failed validation] Form ${validatingElementType} element has invalid background colour => ${opts.assertionSuffix}`);
      }
    }
    if (validatingElement.classList.contains(invalidClass) && element.querySelector(els.cwfFieldErrors) && validatingElementCSS.getPropertyValue('border-color') === invalidBorderColour && validatingElementCSS.getPropertyValue('background-image') === invalidBackgroundImage) {
      return true;
    }
    return false;
  },
  ...
  async wasValidated(arg) {
    return (await this.passedValidation(arg)) || (await this.failedValidation(arg));
  },
  ...
  changesetWebformStateAsJSON(parentSelector, customTransforms) {
    ...
    return Array.from(formElement.querySelectorAll('[data-test-cwf-field]')).map(el => {
      ...
      if (el.querySelector('[data-test-class="cwf-field-label"]')) {
        obj.label = this.removeExtraSpaces(el.querySelector('[data-test-class="cwf-field-label"]').textContent);
      }
      ['radio-button', 'checkbox'].forEach(elType => {
        if (obj.fieldType === `${elType}-group`) {
          const items = Array.from(el.querySelectorAll(`[data-test-labelled-${elType}]`)).map(item => {
            return {
              label: this.removeExtraSpaces(item.querySelector('label').textContent),
      ...
      if (obj.fieldType === 'single-checkbox') {
        obj[camelize(obj.fieldType)] = {
          ...
          // TODO better selector for this.
          checked: el.querySelector('input[type="checkbox"]').checked
        };
      }
      if (['power-select', 'power-select-checkboxes'].includes(obj.fieldType)) {
        const elObject = {
          label: this.removeExtraSpaces(el.querySelector('[data-test-class="cwf-field-label"]').textContent)
        };
        if (el.querySelector('.ember-power-select-placeholder')) {
          elObject.placeholder = this.removeExtraSpaces(el.querySelector('.ember-power-select-placeholder').textContent);
        }
        if (el.querySelector('.ember-power-select-selected-item')) {
          elObject.selectedItemText = this.removeExtraSpaces(el.querySelector('.ember-power-select-selected-item').textContent);
        }
